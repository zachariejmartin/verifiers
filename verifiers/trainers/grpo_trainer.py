...
class GRPOTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        if peft_config is not None:
            ...
            self.logger.info(
                f"[GRPOTrainer] PEFT active: {is_peft_model(model)} "
        ...
        if self.max_prompt_length is not None:
            ...
            train_dataset = train_dataset.filter(
                filter_by_prompt_length, num_proc=self.max_num_processes
            )
    ...
    def _prepare_inputs(  # type: ignore
        ...
        if self._step % generate_every == 0 or self._buffered_inputs is None:
            ...
            for batch_id in range(self._next_batch_id, target_batch_id + 1):
                ...
                if self.accelerator.is_main_process:
                    print(f"submitting batch_id: {batch_id} | batch_offset: {batch_offset} | _step: {self._step} | batch_id_to_retrieve: {batch_id_to_retrieve} | generate_every: {generate_every}")
                    ...
                    self.logger.info(
                        f"Submitted batch {batch_id} with {len(all_prompts)} prompts (num processes: {self.accelerator.num_processes})"
                    )
                    batches_submitted += 1
                self.accelerator.wait_for_everyone()
            ...
            if self.accelerator.is_main_process:
                ...
                if batches_submitted > 0:
                    self.logger.info(
                        f"Submitted {batches_submitted} batches, next_batch_id now {self._next_batch_id}"
                    )
            self.accelerator.wait_for_everyone()
            ...
            if self.accelerator.is_main_process:
                self.logger.info(
                    f"Retrieving batch {batch_id_to_retrieve} for processing"
                )
            ...
            for i in range(process_slice.start, process_slice.stop):
                prompt_ids_list.append(
                    torch.tensor(
                        broadcast_data["prompt_ids"][i], device=self.accelerator.device
                    )
                )
                prompt_mask_list.append(
                    torch.tensor(
                        broadcast_data["prompt_mask"][i], device=self.accelerator.device
                    )
                )
            ...
            if (
                ...
                and prompt_ids.size(1) > self.max_prompt_length
            ):
            ...
            if (
                ...
                and completion_ids.size(1) > self.max_completion_length
            ):
    ...
    def _log_textual_data_primary(
        ...
        self._textual_logs["answer"].extend(all_answers)
